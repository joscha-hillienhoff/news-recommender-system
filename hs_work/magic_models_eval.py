...
def scoring_for_classifier(truth_lines, predictions, sub_files_dict):
    ...
    for line_index, truth_line in enumerate(truth_lines):
        ...
        if sub_impid != impid:
            raise ValueError(f"Line-{line_index+1}: Inconsistent Impression Id {sub_impid} vs {impid}")
...
if __name__ == '__main__':
    ...
    predictions_df = pd.read_csv(r"C:\Users\hryad\Desktop\iitm\Even Sem'25\Recommender Systems\Project\TDT4215-Group-Project\hs_work\all_classifier_predictions.csv")
    ...
    for clf in classifier_columns:
        ...
        if len(predictions) != len(truth_lines):
            raise ValueError(f"Number of predictions in {clf} ({len(predictions)}) does not match number of truth lines ({len(truth_lines)})")
